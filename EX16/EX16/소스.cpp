...
GLvoid Timer(int value)
{
	...
	// 화살표키로 연속 이동 처리 (키를 누른 상태면 매 타이머마다 이동)
	if (moveLeft)  gTranslateX -= translationSpeed;
	if (moveRight) gTranslateX += translationSpeed;
	if (moveUp)    gTranslateY += translationSpeed;
	if (moveDown)  gTranslateY -= translationSpeed;
...
GLvoid drawScene()
{
	...
	if(drawCube) {
		...
		gModel = glm::translate(glm::mat4(1.0f), glm::vec3(gTranslateX, gTranslateY, 0.0f));
	...
	if (drawPyramid){
		...
		// translation 먼저 설정하고 rotation을 적용 -> 오브젝트가 자신의 중심에서 회전하고 이동됨
		gModel = glm::translate(glm::mat4(1.0f), glm::vec3(gTranslateX, gTranslateY, 0.0f));
		gModel = ApplyXRotation(gModel); // Pyramid에만 회전 적용
		gModel = ApplyYRotation(gModel);
